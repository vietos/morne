...
Object.assign(AppProcess.prototype, {
  ...
  start: function () {
    ...
    if (process.send) {
      const listening = ({ type, data }) => {
        self.onListen && self.onListen();
        self.listening = true
        process.sendMessage('listening', {})
      }
      process.onMessage('listening', listening)
    }
    else {
  ...
  _computeEnvironment: function () {
    ...
    process.sendMessage?.('logs', { message: 'PORT:' + self.port })
    env.PORT = self.port;
...
Object.assign(AppRunner.prototype, {
  ...
  _runOnce: function (options) {
    ...
    (appProcess.proc || process).onMessage("shell-server", message => {
      if (message && message.command === "reload") {
        process.sendMessage('logs', { message: 'Reload'})
